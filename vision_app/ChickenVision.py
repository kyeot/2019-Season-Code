    ...
    if len(contours) >= 2:
        ...
        for i in range(len(biggestCnts) - 1):
            ...
            if (np.sign(tilt1) != np.sign(tilt2)):
                xCenterOfTarget = math.floor((cx1 + cx2) / 2)
                yCenterOfTarget = math.floor((cy1 + cy2) / 2)
                ...
                x, y, z = calculateVector(yCenterOfTarget, centerY, xCenterOfTarget, centerX)
                #Make sure no duplicates, then append
                if [xCenterOfTarget, x, y, z] not in targets:
                    targets.append([xCenterOfTarget, x, y, z])
